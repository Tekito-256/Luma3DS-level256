...
static bool dirCheck(FS_ArchiveID archiveId, const char *path)
{
    ...
    if (R_FAILED(FSUSER_OpenArchive(&archive, archiveId, fsMakePath(PATH_EMPTY, ""))))
        ret = false;
    else
    {
...
static inline void loadCustomVerString(u16 *out, u32 *verStringSize, u32 currentNand)
{
    ...
    if (!openLumaFile(&file, paths[currentNand]))
        return;

    u64 fileSize;

    if (R_FAILED(IFile_GetSize(&file, &fileSize)) || fileSize > 62)
        goto exit;
...
static inline bool findLayeredFsSymbols(u8 *code, u32 size, u32 *fsMountArchive, u32 *fsRegisterArchive, u32 *fsTryOpenFile, u32 *fsOpenFileDirectly)
{
    ...
    for (u32 addr = 0; addr <= size - 4; addr += 4)
    {
        ...
        {
        case 0xE5970010:
            if (addr <= size - 12 && *fsMountArchive == 0xFFFFFFFF && addr32[1] == 0xE1CD20D8 && (addr32[2] & 0xFFFFFF) == 0x008D0000)
                temp = fsMountArchive;
            break;
        case 0xE24DD028:
            if (addr <= size - 16 && *fsMountArchive == 0xFFFFFFFF && addr32[1] == 0xE1A04000 && addr32[2] == 0xE59F60A8 && addr32[3] == 0xE3A0C001)
                temp = fsMountArchive;
            break;
        case 0xE3500008:
            if (addr <= size - 12 && *fsRegisterArchive == 0xFFFFFFFF && (addr32[1] & 0xFFF00FF0) == 0xE1800400 && (addr32[2] & 0xFFF00FF0) == 0xE1800FC0)
                temp = fsRegisterArchive;
            break;
        case 0xE351003A:
            if (addr <= size - 0x40 && *fsTryOpenFile == 0xFFFFFFFF && addr32[1] == 0x1AFFFFFC && addr32[0xD] == 0xE590C000 && addr32[0xF] == 0xE12FFF3C)
                temp = fsTryOpenFile;
            break;
        case 0x08030204:
            if (*fsOpenFileDirectly == 0xFFFFFFFF)
                temp = fsOpenFileDirectly;
            break;
        }
...
static inline bool findLayeredFsPayloadOffset(u8 *code, u32 size, u32 roSize, u32 dataSize, u32 roAddress, u32 dataAddress, u32 *payloadOffset, u32 *pathOffset, u32 *pathAddress)
{
    ...
    if (roundedTextSize - size >= romfsRedirPatchSize)
        *payloadOffset = size;
    else
    {
...
static inline bool applyCodeIpsPatch(u64 progId, u8 *code, u32 size)
{
    ...
    if (R_FAILED(IFile_Read(&file, &total, buffer, 5)) || total != 5 || memcmp(buffer, "PATCH", 5) != 0)
        goto exit;
    ...
    while (R_SUCCEEDED(IFile_Read(&file, &total, buffer, 3)) && total == 3)
    {
        ...
        if (R_FAILED(IFile_Read(&file, &total, buffer, 2)) || total != 2)
            break;
        ...
        if (!patchSize)
        {
            ...
            if (offset + rleSize > size)
                break;

            if (R_FAILED(IFile_Read(&file, &total, buffer, 1)) || total != 1)
                break;
        ...
        if (offset + patchSize > size)
            break;

        if (R_FAILED(IFile_Read(&file, &total, code + offset, patchSize)) || total != patchSize)
            break;
    }
...
bool loadTitleCodeSection(u64 progId, u8 *code, u32 size)
{
    ...
    if (R_FAILED(IFile_GetSize(&file, &fileSize)) || fileSize > size)
        goto error;
    else
    {
        u64 total;

        if (R_FAILED(IFile_Read(&file, &total, code, fileSize)) || total != fileSize)
            goto error;
    }
...
bool loadTitleExheaderInfo(u64 progId, ExHeader_Info *exheaderInfo)
{
    ...
    if (R_FAILED(IFile_GetSize(&file, &fileSize)) || (fileSize != sizeof(ExHeader_Info) && fileSize != sizeof(ExHeader)))
        goto error;
    else
    {
        u64 total;

        if (R_FAILED(IFile_Read(&file, &total, exheaderInfo, sizeof(ExHeader_Info))) || total != sizeof(ExHeader_Info))
            goto error;
    }
...
static inline bool loadTitleLocaleConfig(u64 progId, u8 *mask, u8 *regionId, u8 *languageId, u8 *countryId, u8 *stateId)
{
    ...
    *mask = *regionId = *languageId = *countryId = *stateId = 0;
    ...
    if (R_FAILED(IFile_GetSize(&file, &fileSize)) || fileSize < 3)
        goto exit;
    if (fileSize >= 12)
        fileSize = 12;
        ...
        ((buf[11] >= '0' && buf[11] <= '9') || (buf[11] >= 'a' && buf[11] <= 'f') || (buf[11] >= 'A' && buf[11] <= 'F')))
    {
        if (buf[10] >= '0' && buf[10] <= '9')
            *stateId = 16 * (buf[10] - '0');
        else if (buf[10] >= 'a' && buf[10] <= 'f')
            *stateId = 16 * (buf[10] - 'a' + 10);
        else if (buf[10] >= 'A' && buf[10] <= 'F')
            *stateId = 16 * (buf[10] - 'A' + 10);

        if (buf[11] >= '0' && buf[11] <= '9')
            *stateId += buf[11] - '0';
        else if (buf[11] >= 'a' && buf[11] <= 'f')
            *stateId += buf[11] - 'a' + 10;
        else if (buf[11] >= 'A' && buf[11] <= 'F')
            *stateId += buf[11] - 'A' + 10;
...
static inline bool patchLayeredFs(u64 progId, u8 *code, u32 size, u32 textSize, u32 roSize, u32 dataSize, u32 roAddress, u32 dataAddress)
{
    ...
    if (!findLayeredFsSymbols(code, textSize, &fsMountArchive, &fsRegisterArchive, &fsTryOpenFile, &fsOpenFileDirectly) ||
        !findLayeredFsPayloadOffset(code, textSize, roSize, dataSize, roAddress, dataAddress, &payloadOffset, &pathOffset, &pathAddress))
        return false;
    ...
    else
    {
        ...
        for (updateRomFsIndex = 0; updateRomFsIndex < sizeof(updateRomFsMounts) / sizeof(char *) - 1; updateRomFsIndex++)
        {
            ...
            if (memsearch(code, temp, size, patternSize + 1) != NULL)
                break;
        }
...
void patchCode(u64 progId, u16 progVer, u8 *code, u32 size, u32 textSize, u32 roSize, u32 dataSize, u32 roAddress, u32 dataAddress)
{
    ...
    if (isHomeMenu)
    {
        ...
        if (applyRegionFreePatch)
        {
            static const u8 pattern[] = {
                0x0A, 0x0C, 0x00, 0x10},
                            patch[] = {0x01, 0x00, 0xA0, 0xE3, 0x1E, 0xFF, 0x2F, 0xE1};
    ...
    else if (progId == 0x0004013000003702LL && progVer > 0) // RO
    {
        static const u8 pattern[] = {
            0x20, 0xA0, 0xE1, 0x8B},
                        pattern2[] = {0xE1, 0x30, 0x40, 0x2D}, pattern3[] = {0x2D, 0xE9, 0x01, 0x70}, patch[] = {
    ...
    {
        if (!patcherApplyCodeBpsPatch(progId, code, size))
            goto error;
        if (!applyCodeIpsPatch(progId, code, size))
            goto error;
    }

    if (CONFIG(PATCHGAMES) && !(isApp && nextGamePatchDisabled) && !isPatchProhibitedTitle(progId))
    {
        if (!isSysmodule)
        {
            if (!patcherApplyCodeBpsPatch(progId, code, size))
        ...
        if (isApp || isApplet)
        {
            ...
            if (!patchLayeredFs(progId, code, size, textSize, roSize, dataSize, roAddress, dataAddress))
                goto error;
        }
    }
